...
export class TestSeeder {
  ...
  async seed() {
    ...
    const psychometricTest = await this.createTestIfNotExists({
      ...
      description: 'Evaluación de habilidades cognitivas, razonamiento lógico y personalidad. Este test evalúa diferentes aspectos cognitivos. Lee cada pregunta cuidadosamente y selecciona la respuesta que mejor represente tu opinión. Tienes 60 minutos para completar el test.',
    ...
    if (psychometricTest) {
      await this.createQuestions(psychometricTest, [
        {
          question: '¿Cuál es el siguiente número en la secuencia? 2, 4, 8, 16, __',
        ...
        {
          question: 'Si todos los gatos son animales y algunos animales son mascotas, entonces:',
          type: QuestionType.MULTIPLE_CHOICE,
          options: [
        ...
        {
          question: 'Completa la analogía: Día es a Noche como Verano es a ____',
          type: QuestionType.MULTIPLE_CHOICE,
    ...
    const technicalTest = await this.createTestIfNotExists({
      ...
      description: 'Evaluación de conocimientos técnicos para operadores de equipos mineros. Responde basándote en tus conocimientos y experiencia. Cada pregunta tiene solo una respuesta correcta. Tienes 45 minutos para completar el test. Debes obtener al menos 75% para aprobar.',
    ...
    if (technicalTest) {
      await this.createQuestions(technicalTest, [
        {
          question: '¿Cuál es la función principal de un sistema hidráulico en equipos mineros?',
          type: QuestionType.MULTIPLE_CHOICE,
          options: [
        ...
        {
          question: '¿Qué indica una luz roja en el tablero de un camión minero?',
          type: QuestionType.MULTIPLE_CHOICE,
          options: [
            'Advertencia menor',
        ...
        {
          question: 'Antes de iniciar la operación, ¿qué inspección es obligatoria?',
          type: QuestionType.MULTIPLE_CHOICE,
          options: [
        ...
        {
          question: '¿Cuál es el procedimiento correcto ante una falla del sistema de frenos?',
          type: QuestionType.MULTIPLE_CHOICE,
          options: [
    ...
    const safetyTest = await this.createTestIfNotExists({
      ...
      description: 'Evaluación de conocimientos en seguridad y prevención de riesgos. La seguridad es prioritaria en minería. Responde basándote en normativas vigentes. Debes obtener al menos 80% para aprobar. Tienes 30 minutos.',
      ...
      await this.createQuestions(safetyTest, [
        {
          ...
          correctAnswers: ['Casco, zapatos de seguridad, lentes, guantes y protección auditiva'],
          points: 10,
          order: 1,
        },
        {
          question: '¿Qué significa el triángulo amarillo con signo de exclamación?',
          type: QuestionType.MULTIPLE_CHOICE,
          options: [
            'Prohibición',
  ...
  private async createTestIfNotExists(testData: Partial<Test>): Promise<Test | null> {
  ...
  private async createQuestions(test: Test, questions: Partial<TestQuestion>[]) {
    ...
    console.log(`   ✅ ${questions.length} preguntas creadas para: ${test.name}`);
  }
}
